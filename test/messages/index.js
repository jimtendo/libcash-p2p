...
describe('Messages', function() {
  ...
  describe('@constructor', function() {
    it('sets properties correctly', function() {
      var network = libcash.Networks.defaultNetwork;
      var messages = new Messages({
        network: network,
        Block: libcash.Block,
      ...
      messages.builder.constructors.Block.should.equal(libcash.Block);
      messages.builder.constructors.Transaction.should.equal(libcash.Transaction);
      messages.network.should.deep.equal(network);
    });
    it('network should be unique for each set of messages', function() {
      var messages = new Messages({
        network: libcash.Networks.livenet
      });
      var messages2 = new Messages({
        network: libcash.Networks.testnet
      });
      messages.network.should.deep.equal(libcash.Networks.livenet);
      messages2.network.should.deep.equal(libcash.Networks.testnet);
      var message1 = messages.Version();
      message1.network.should.deep.equal(libcash.Networks.livenet);
      var message2 = messages2.Version();
      message2.network.should.deep.equal(libcash.Networks.testnet);
    });
  });
  ...
  describe('Default Network', function() {
    ...
    Object.keys(messages.builder.commandsMap).forEach(function(command) {
      ...
      it(name, function() {
        var message = messages[name]();
        message.network.should.deep.equal(libcash.Networks.defaultNetwork);
      });
    });
